...
exports.sendOTP = async ({ to, otp }) => {
  try {
        ...
        <div style="max-width: 500px; margin: 0 auto; background: #ffffff; border-radius: 12px; box-shadow: 0 2px 8px rgba(0,0,0,0.1); overflow: hidden;">
          ...
          <div style="padding: 30px; color: #333;">
            ...
            <p style="font-size: 15px; line-height: 1.6;">
              Use the verification code below to complete your registration on <strong>SwiftOra</strong>.
            </p>
          ...
          <div style="background: #f0f4f8; text-align: center; padding: 15px; font-size: 12px; color: #888;">
            © ${new Date().getFullYear()} SwiftOra. All rights reserved.<br/>
            <a href="https://Swiftora.store" style="color: #009900; text-decoration: none;">Visit Website</a>
          </div>
        </div>
      </div>
    `;
    ...
    const data = await resend.emails.send({
      from: process.env.FROM_EMAIL, // no-reply@swiftora.store
      to,
      subject: "Your Swiftora OTP Code",
      html: htmlContent
    });
